...
// Mock supabase for preview — real supabase.js file handles this in your local project
const supabase = {
...
const COLORS = {
  navy: "#0A1628", navyLight: "#0F2040", blue: "#1E6FD9", blueLight: "#3B8BF5",
  orange: "#F97316", white: "#FFFFFF", gray50: "#F8FAFC", gray100: "#F1F5F9",
...
const storefront = {
  logo: "G", name: "Greenville Tire Pros", phone: "(864) 555-0142",
  address: "1420 Wade Hampton Blvd, Greenville, SC 29609",
  hours: "Mon–Fri 8am–6pm · Sat 8am–4pm", primaryColor: "#1E6FD9",
  hero: "Greenville's Trusted Tire Experts", heroBg: "#0A1628",
...
const S = {
  ...
  badge: (color) => {
    const map = { New: ["#EFF6FF","#1D4ED8"], Reviewing: ["#FEF9C3","#854D0E"], Approved: ["#DCFCE7","#166534"], Waitlisted: ["#FEF3C7","#92400E"], Rejected: ["#FEE2E2","#991B1B"], Invited: ["#EDE9FE","#5B21B6"], Active: ["#DCFCE7","#166534"], Trial: ["#DBEAFE","#1E40AF"], "Past Due": ["#FEF3C7","#92400E"], Suspended: ["#FEE2E2","#991B1B"], Cancelled: ["#F3F4F6","#374151"], Pending: ["#FEF9C3","#854D0E"], Confirmed: ["#D1FAE5","#065F46"], Completed: ["#DBEAFE","#1E40AF"], Open: ["#DCFCE7","#166534"], Limited: ["#FEF9C3","#854D0E"], Full: ["#FEE2E2","#991B1B"], "Waitlist Only": ["#EDE9FE","#5B21B6"], "Out of Stock": ["#F3F4F6","#374151"] };
...
function LandingPage({ nav }) {
  ...
  const localPlans = [
    { name: "Early Partner", price: 149, highlight: false, paymentLink: "https://buy.stripe.com/test_28EdR85bp3oDczze9K8so02", features: ["Online tire storefront", "Inventory dashboard", "Online reservations", "Order management", "Basic SEO pages", "Email notifications"] },
    ...
    { name: "Market Leader", price: 399, highlight: false, paymentLink: "https://buy.stripe.com/test_5kQbJ033hcZd4333v68so00", features: ["Everything in Growth", "AI chatbot", "Custom domain support", "Promotions & coupons", "Advanced reporting", "Multi-location support", "Priority onboarding"] },
  ];
  const faqs = [
...
function InvitePage({ nav }) {
  ...
  const submitApplication = async () => {
    const { error } = await supabase.from('invite_applications').insert({
      shop_name: form.shopName, owner_name: form.ownerName, email: form.email,
      phone: form.phone, city: form.city, state: form.state, status: 'New'
    });
      ...
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: "36px 36px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div><label style={S.label}>Number of Locations</label><select style={{...S.select, width:"100%"}} value={form.locations} onChange={e => set("locations", e.target.value)}>{["1","2","3","4","5+"].map(v => <option key={v}>{v}</option>)}</select></div>
            <div><label style={S.label}>Tires Sold</label><select style={{...S.select, width:"100%"}} value={form.tireType} onChange={e => set("tireType", e.target.value)}>{["New","Used","Both"].map(v => <option key={v}>{v}</option>)}</select></div>
            <div><label style={S.label}>Approx. Tires in Inventory</label><select style={{...S.select, width:"100%"}} value={form.inventory} onChange={e => set("inventory", e.target.value)}>{["Under 50","50–100","100–200","200–500","500+"].map(v => <option key={v}>{v}</option>)}</select></div>
            <div><label style={S.label}>Current Inventory Method</label><select style={{...S.select, width:"100%"}} value={form.currentMethod} onChange={e => set("currentMethod", e.target.value)}>{["Spreadsheets","Paper","POS Software","None","Other"].map(v => <option key={v}>{v}</option>)}</select></div>
            <div><label style={S.label}>Accept Online Orders Now?</label><select style={{...S.select, width:"100%"}} value={form.online} onChange={e => set("online", e.target.value)}>{["Yes","No"].map(v => <option key={v}>{v}</option>)}</select></div>
            <div><label style={S.label}>Offer Installation?</label><select style={{...S.select, width:"100%"}} value={form.installation} onChange={e => set("installation", e.target.value)}>{["Yes","No"].map(v => <option key={v}>{v}</option>)}</select></div>
          </div>
...
function SuperAdmin({ nav }) {
  ...
  const updateAppStatus = (id, status) => { setApps(a => a.map(x => x.id === id ? {...x, status} : x)); showToast(`Status updated to ${status}`); setSelectedApp(null); };
  const filteredApps = appFilter === "All" ? apps : apps.filter(a => a.status === appFilter);
  const sidebar = [["overview","📊","Overview"],["applications","📋","Applications"],["shops","🏪","Shops"],["markets","📍","Markets"],["design","🎨","Design Studio"],["plans","💳","Plans & Billing"],["orders","📦","Orders"],["settings","⚙️","Settings"]];
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "system-ui, sans-serif", background: COLORS.gray50 }}>
      ...
      <div style={{ width: 220, background: COLORS.navy, padding: "20px 12px", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 8px 24px" }}>
          ...
          <div><div style={{ color: "#fff", fontWeight: 700, fontSize: 14 }}>TreadFlow</div><div style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>Super Admin</div></div>
        </div>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 28 }}>
        {section === "overview" && <AdminOverview shops={shops} apps={apps} setSection={setSection} />}
        ...
        {section === "shops" && <ShopsList shops={shops} onDesign={s => { setDesignShop(s); setSection("design"); }} onView={() => nav("storefront")} showToast={showToast} />}
...
function AdminOverview({ shops, apps, setSection }) {
  ...
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 28 }}>{metrics.map(m => <MetricCard key={m.label} {...m} />)}</div>
...
function ApplicationsList({ apps, allApps, filter, setFilter, onSelect }) {
  ...
  return <div>
    <div style={{ marginBottom: 20 }}><h2 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Applications</h2><p style={{ color: COLORS.gray500, marginTop: 4 }}>{allApps.length} total applications</p></div>
    ...
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead><tr>{["Shop","Owner","Location","Plan","Status","Date",""].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
  ...
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20 }}>
      <div style={S.card}>
        ...
        <div style={{ color: COLORS.gray500, fontSize: 14, marginBottom: 20 }}>Submitted {app.date}</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {[["Owner",app.owner],["Email",app.email],["Phone",app.phone],["Location",`${app.city}, ${app.state}`],["Tire Types",app.tires],["Inventory",app.inventory],["Market",app.market],["Status",<span style={S.badge(app.status)}>{app.status}</span>]].map(([k,v]) => <div key={k}>
        ...
        <div style={{ marginTop: 20 }}><label style={S.label}>Internal Notes</label><textarea style={{ ...S.input, height: 80, resize: "vertical" }} value={note} onChange={e => setNote(e.target.value)} placeholder="Add review notes..." /></div>
      </div>
      <div style={S.card}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <button onClick={() => onAction(app.id, "Approved")} style={{ ...S.btn("primary"), justifyContent: "center" }}>✓ Approve</button>
          <button onClick={() => onAction(app.id, "Invited")} style={{ ...S.btn("primary"), justifyContent: "center", background: COLORS.purple }}>✉ Send Invite</button>
        ...
        {app.status === "Approved" && <div style={{ marginTop: 16, background: "#F0FDF4", borderRadius: 8, padding: "12px 14px" }}>
          ...
          <div style={{ fontFamily: "monospace", fontSize: 16, fontWeight: 700, letterSpacing: 2 }}>TF-SC-{Math.random().toString(36).substring(2,8).toUpperCase()}</div>
...
function ShopsList({ shops, onDesign, onView, showToast }) {
  return <div>
    <div style={{ marginBottom: 20 }}><h2 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Shops</h2><p style={{ color: COLORS.gray500, marginTop: 4 }}>{shops.length} shops on platform</p></div>
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
...
function MarketsPage({ showToast }) {
  return <div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
      ...
      <button onClick={() => showToast("Add market coming soon")} style={S.btn("primary")}>+ Add Market</button>
    </div>
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead><tr>{["Market","City","State","Capacity","Fill","Status","Actions"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
        <tbody>{mockMarkets.map(m => <tr key={m.id}>
          <td style={S.td}><div style={{ fontWeight: 600 }}>{m.name}</div></td>
          <td style={S.td}>{m.city}</td><td style={S.td}>{m.state}</td>
...
function StorefrontStudio({ shop, shops, onShopChange, showToast }) {
  ...
  return <div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
      <div><h2 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Storefront Design Studio</h2><p style={{ color: COLORS.gray500, marginTop: 4, fontSize: 14 }}>Customize storefronts for each shop client</p></div>
      <div style={{ display: "flex", gap: 10 }}>
        <select style={S.select} value={shop?.name} onChange={e => onShopChange(shops.find(s => s.name === e.target.value))}>{shops.map(s => <option key={s.id}>{s.name}</option>)}</select>
        <button onClick={() => showToast("Draft saved")} style={S.btn("secondary")}>Save Draft</button>
        <button onClick={() => showToast("Published live!")} style={S.btn("primary")}>Publish Live →</button>
      </div>
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 20 }}>
      <div style={S.card}>
        <div style={{ display: "flex", gap: 6, marginBottom: 18, borderBottom: "1px solid #E2E8F0", paddingBottom: 14 }}>
          {["brand","content","layout","sections"].map(t => <button key={t} onClick={() => setTab(t)} style={{ fontSize: 13, padding: "5px 12px", borderRadius: 7, cursor: "pointer", border: "none", background: tab === t ? COLORS.navy : COLORS.gray100, color: tab === t ? "#fff" : COLORS.gray600, fontWeight: tab === t ? 600 : 400, textTransform: "capitalize" }}>{t}</button>)}
        </div>
        {tab === "brand" && <div>
          <label style={S.label}>Template</label>
          <select style={{ ...S.select, width: "100%", marginBottom: 14 }} value={template} onChange={e => setTemplate(e.target.value)}>{templates.map(t => <option key={t}>{t}</option>)}</select>
        ...
        {tab === "content" && <div>
          ...
          <input style={{ ...S.input, marginBottom: 12 }} defaultValue="Free installation on sets of 4!" />
        </div>}
        {tab === "layout" && <div>
          ...
          {["Can edit business info","Can edit services","Can edit homepage text","Can edit promotions"].map(p => <label key={p} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8, fontSize: 14, color: COLORS.gray700, cursor: "pointer" }}><input type="checkbox" defaultChecked style={{ accentColor: COLORS.blue }} />{p}</label>)}
        </div>}
        {tab === "sections" && <div>
          ...
          {sections.map(s => <div key={s} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <label style={{ position: "relative", display: "inline-block", width: 36, height: 20, cursor: "pointer" }}>
              ...
              <span style={{ position: "absolute", inset: 0, background: vis[s] ? COLORS.blue : COLORS.gray300, borderRadius: 99 }}><span style={{ position: "absolute", left: vis[s] ? 18 : 2, top: 2, width: 16, height: 16, background: "#fff", borderRadius: 99, transition: "0.2s" }} /></span>
            </label>
          </div>)}
        </div>}
      </div>
      <div style={S.card}>
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          ...
          <div style={{ marginLeft: "auto", fontSize: 12, color: COLORS.gray400, alignSelf: "center" }}>Preview · {shop?.name}</div>
        </div>
        <div style={{ background: "#F8FAFC", borderRadius: 8, padding: 10 }}>
          <div style={{ maxWidth: preview === "desktop" ? "100%" : preview === "tablet" ? 480 : 320, margin: "0 auto", background: "#fff", borderRadius: 8, border: "1px solid #E2E8F0", overflow: "hidden" }}>
            <div style={{ background: primary, padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>{["Inventory","Services","Contact"].map(l => <span key={l} style={{ color: "rgba(255,255,255,0.7)", fontSize: 11 }}>{l}</span>)}</div>
            </div>
            {vis["Hero"] && <div style={{ background: COLORS.navy, padding: "24px 18px", textAlign: "center" }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", marginBottom: 6 }}>{hero}</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginBottom: 12 }}>{heroSub.substring(0,80)}...</div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          <button onClick={() => showToast("Version saved")} style={{ ...S.btn("secondary", "sm") }}>Save Version</button>
          <button onClick={() => showToast("Reverted")} style={{ ...S.btn("secondary", "sm") }}>Revert</button>
          <div style={{ marginLeft: "auto" }}><button onClick={() => showToast("Published!")} style={S.btn("primary")}>Publish Changes →</button></div>
        </div>
      </div>
    </div>
  </div>;
...
function PlansPage() {
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
      {[{name:"Early Partner",price:149},{name:"Growth Partner",price:249},{name:"Market Leader",price:399}].map(p => <div key={p.name} style={S.card}>
    ...
    <div style={{ ...S.card, marginTop: 20 }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {mockShops.map(s => <tr key={s.id}>
            <td style={S.td}>{s.name}</td><td style={S.td}>{s.plan}</td>
            <td style={{ ...S.td, color: COLORS.green, fontWeight: 700 }}>${s.mrr}</td>
            <td style={S.td}><span style={S.badge(s.status)}>{s.status}</span></td>
            <td style={S.td}>Jun 1, 2026</td>
          </tr>)}
          <tr style={{ background: COLORS.gray50 }}>
            <td style={{ ...S.td, fontWeight: 700 }}>Total MRR</td><td style={S.td}></td>
    ...
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>{mockOrders.map(o => <tr key={o.id}>
          ...
          <td style={S.td}>{o.customer}</td><td style={S.td}>Greenville Tire Pros</td>
...
function ShopDashboard({ nav }) {
  ...
  const showToast = msg => { setToast(msg); setTimeout(() => setToast(null), 2800); };
  const sidebar = [["overview","📊","Overview"],["inventory","📦","Inventory"],["orders","📋","Orders"],["appointments","📅","Appointments"],["customers","👥","Customers"],["staff","👤","Staff"],["settings","⚙️","Settings"],["billing","💳","Billing"]];
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "system-ui, sans-serif", background: COLORS.gray50 }}>
      {toast && <div style={{ position: "fixed", bottom: 24, right: 24, background: COLORS.gray900, color: "#fff", padding: "12px 20px", borderRadius: 10, fontSize: 14, zIndex: 999 }}>{toast}</div>}
      <div style={{ width: 220, background: "#0A1628", padding: "20px 12px", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 8px 24px" }}>
          ...
          <div><div style={{ color: "#fff", fontWeight: 700, fontSize: 13 }}>Greenville Tire Pros</div><div style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>Shop Dashboard</div></div>
        </div>
...
function ShopOverview({ tires, orders }) {
  ...
  return <div>
    <div style={{ marginBottom: 24 }}><h2 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Dashboard</h2><p style={{ color: COLORS.gray500, marginTop: 4 }}>Greenville Tire Pros · Greenville, SC</p></div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
      ...
      <MetricCard label="Est. Revenue" value={`$${revenue.toFixed(0)}`} color={COLORS.green} />
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <div style={S.card}>
        ...
        {mockOrders.slice(0,4).map(o => <div key={o.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #F1F5F9" }}>
          <div><div style={{ fontSize: 14, fontWeight: 600 }}>{o.customer}</div><div style={{ fontSize: 12, color: COLORS.gray400 }}>{o.id}</div></div>
...
function InventoryPage({ tires, setTires, showToast, selectedTire, setSelectedTire }) {
  ...
  const addTire = () => {
    setTires(ts => [...ts, { ...newTire, id: Date.now(), qty: +newTire.qty, price: +newTire.price, status: "Active", featured: false, installFee: 25, disposalFee: 5, setPrice: (+newTire.price*4), dot: "", load: 97, speed: "H", images: [] }]);
    showToast("Tire added"); setShowAdd(false);
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>
      <div style={S.card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[["Condition",selectedTire.condition],["Quantity",selectedTire.qty],["Tread",selectedTire.tread||"N/A"],["DOT",selectedTire.dot||"—"],["Load",selectedTire.load],["Speed",selectedTire.speed],["Install Fee","$"+selectedTire.installFee],["Disposal","$"+selectedTire.disposalFee],["Status",selectedTire.status]].map(([k,v]) => <div key={k} style={{ background: COLORS.gray50, borderRadius: 8, padding: "10px 12px" }}>
      ...
      <div style={S.card}>
        ...
        <div style={{ marginBottom: 10 }}><label style={S.label}>Price Per Tire</label><input style={S.input} defaultValue={`$${selectedTire.price}`} /></div>
        <div style={{ marginBottom: 10 }}><label style={S.label}>Set Price (4 tires)</label><input style={S.input} defaultValue={`$${selectedTire.setPrice}`} /></div>
        <div style={{ marginBottom: 16 }}><label style={S.label}>Quantity</label><input type="number" style={S.input} defaultValue={selectedTire.qty} /></div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <button onClick={() => { showToast("Tire updated"); setSelectedTire(null); }} style={{ ...S.btn("primary"), justifyContent: "center" }}>Save Changes</button>
          <button onClick={() => { setTires(ts => ts.map(t => t.id === selectedTire.id ? {...t, featured: !t.featured} : t)); showToast("Updated"); }} style={{ ...S.btn("secondary"), justifyContent: "center" }}>{selectedTire.featured ? "Remove Featured" : "Mark Featured"}</button>
          <button onClick={() => { setTires(ts => ts.filter(t => t.id !== selectedTire.id)); showToast("Deleted"); setSelectedTire(null); }} style={{ ...S.btn("danger"), justifyContent: "center" }}>Delete Tire</button>
        </div>
      </div>
    </div>
  </div>;
  return <div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
      <div><h2 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Inventory</h2><p style={{ color: COLORS.gray500, marginTop: 4 }}>{tires.reduce((a,t)=>a+t.qty,0)} tires in stock</p></div>
      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={() => showToast("CSV upload opened")} style={S.btn("secondary")}>📤 CSV Upload</button>
    ...
    {showAdd && <div style={{ ...S.card, marginBottom: 20, background: "#F0F7FF", border: "1px solid #93C5FD" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        {[["brand","Brand"],["model","Model"],["size","Size"],["price","Price"]].map(([k,l]) => <div key={k}><label style={S.label}>{l}</label><input style={S.input} value={newTire[k]} onChange={e => setNewTire(t => ({...t,[k]:e.target.value}))} /></div>)}
        ...
        <div><label style={S.label}>Tread Depth</label><input style={S.input} value={newTire.tread} onChange={e => setNewTire(t => ({...t,tread:e.target.value}))} placeholder="e.g. 8/32" /></div>
      </div>
    ...
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead><tr>{["Tire","Size","Cond.","Type","Qty","Price","Set Price","Status",""].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
        <tbody>{filtered.map(t => <tr key={t.id}>
          <td style={S.td} onClick={() => setSelectedTire(t)} style={{ ...S.td, cursor: "pointer" }}><div style={{ fontWeight: 600 }}>{t.brand} {t.model}</div>{t.featured && <span style={{ fontSize: 11, background: "#FEF9C3", color: "#854D0E", padding: "1px 6px", borderRadius: 4 }}>Featured</span>}</td>
...
function OrdersPage({ orders, setOrders, showToast }) {
  ...
  return <div>
    <div style={{ marginBottom: 20 }}><h2 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Orders</h2><p style={{ color: COLORS.gray500, marginTop: 4 }}>{orders.length} orders total</p></div>
    <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {filtered.map(o => <div key={o.id} style={{ ...S.card, display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 16, alignItems: "center" }}>
        <div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}><span style={{ fontWeight: 700, color: COLORS.blue }}>{o.id}</span><span style={S.badge(o.status)}>{o.status}</span></div>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: COLORS.gray500 }}>Qty: {o.qty} · {o.vehicle}</div>
          {o.apptDate && <div style={{ fontSize: 12, color: COLORS.blue, marginTop: 2 }}>📅 {o.apptDate}</div>}
        </div>
        <div><div style={{ fontSize: 22, fontWeight: 800, color: COLORS.green }}>${o.total}</div><div style={{ fontSize: 12, color: COLORS.gray400 }}>{o.date}</div></div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {o.status === "Confirmed" && <button onClick={() => updateStatus(o.id,"Completed")} style={{ ...S.btn("primary","sm"), justifyContent: "center" }}>Complete</button>}
        </div>
      </div>)}
    </div>
  </div>;
  ...
  return <div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {appts.map(o => <div key={o.id} style={{ ...S.card, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ textAlign: "right" }}><span style={S.badge(o.status)}>{o.status}</span><div style={{ fontSize: 13, color: COLORS.gray400, marginTop: 4 }}>{o.id}</div></div>
      </div>)}
    </div>
  </div>;
}
...
function StaffPage({ showToast }) {
  const staff = [
    { name: "Marcus Williams", role: "Owner", email: "marcus@greenvilletire.com" },
    { name: "Deja Lawson", role: "Manager", email: "deja@greenvilletire.com" },
    { name: "Trevor Banks", role: "Inventory Staff", email: "trevor@greenvilletire.com" },
  ];
  const roles = { Owner: "Full access", Manager: "Inventory, orders, appointments", "Inventory Staff": "Inventory only" };
  return <div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
    ...
    {staff.map(s => <div key={s.name} style={{ ...S.card, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
      <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
        ...
        <div><div style={{ fontWeight: 600 }}>{s.name}</div><div style={{ fontSize: 13, color: COLORS.gray500 }}>{s.email}</div></div>
      </div>
  ...
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <div style={S.card}>
        ...
        <button onClick={() => showToast("Saved!")} style={S.btn("primary")}>Save Changes</button>
      </div>
      <div style={S.card}>
  ...
  return <div>
    ...
    <div style={{ ...S.card, maxWidth: 480 }}>
      ...
      <div style={{ borderTop: "1px solid #E2E8F0", marginTop: 16, paddingTop: 16, fontSize: 14, color: COLORS.gray500 }}>
        Next billing: <strong>June 1, 2026</strong> · Member since November 2025
      </div>
    </div>
  </div>;
}
...
function Storefront({ nav }) {
  ...
  const sendChat = () => {
    ...
    let reply = "I'm not sure about that. Please call us at (864) 555-0142!";
    ...
    else if (msg.includes("reserve") || msg.includes("order")) reply = "Click 'Reserve Now' on any tire card to get started!";
    else if (msg.includes("tire")) reply = "We have new and used tires in stock! Use the search above to find your size.";
  ...
  if (orderDone) return (
    <div style={{ minHeight: "100vh", background: "#F0FDF4", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "60px 48px", textAlign: "center", maxWidth: 480 }}>
        ...
        <p style={{ color: COLORS.gray500, fontSize: 16, lineHeight: 1.7, marginBottom: 24 }}>We've received your tire reservation. You'll get a confirmation email shortly.</p>
    ...
    <div style={{ minHeight: "100vh", background: COLORS.gray50, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 680, margin: "40px auto", padding: "0 20px" }}>
        ...
        <div style={{ ...S.card, marginBottom: 20, display: "flex", gap: 16, alignItems: "center" }}>
          ...
          <div><div style={{ fontWeight: 700 }}>{orderTire.brand} {orderTire.model}</div><div style={{ fontSize: 14, color: COLORS.gray500 }}>{orderTire.size} · {orderTire.condition}</div><div style={{ fontWeight: 800, color: COLORS.blue, fontSize: 18 }}>${orderTire.price}/tire</div></div>
        </div>
        <div style={S.card}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {[["Full Name"],["Phone Number"],["Email Address"],["Vehicle (Year Make Model)"]].map(([l]) => <div key={l} style={l.includes("Vehicle") ? { gridColumn: "1/-1" } : {}}><label style={S.label}>{l}</label><input style={S.input} /></div>)}
            <div><label style={S.label}>Quantity</label><select style={{ ...S.select, width:"100%" }}>{[1,2,3,4].map(n => <option key={n}>{n}</option>)}</select></div>
            <div><label style={S.label}>Service Type</label><select style={{ ...S.select, width:"100%" }}><option>Installation at Shop</option><option>Pickup Only</option></select></div>
            <div><label style={S.label}>Preferred Date</label><input type="date" style={S.input} /></div>
            <div><label style={S.label}>Preferred Time</label><select style={{ ...S.select, width:"100%" }}>{["8:00 AM","9:00 AM","10:00 AM","11:00 AM","1:00 PM","2:00 PM","3:00 PM"].map(t => <option key={t}>{t}</option>)}</select></div>
            <div style={{ gridColumn: "1/-1" }}><label style={S.label}>Payment Option</label><select style={{ ...S.select, width:"100%" }}><option>Pay deposit online ($50)</option><option>Pay in full online</option><option>Pay at shop</option></select></div>
            <div style={{ gridColumn: "1/-1" }}><label style={S.label}>Notes</label><textarea style={{ ...S.input, height: 60, resize: "vertical" }} /></div>
          </div>
  ...
  if (selectedTire) return (
    <div style={{ minHeight: "100vh", background: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 900, margin: "32px auto", padding: "0 20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
        <div style={{ background: COLORS.gray100, borderRadius: 16, height: 300, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 80 }}>🛞</div>
        <div>
          ...
          <div style={{ fontSize: 18, color: COLORS.green, fontWeight: 700 }}>Set of 4: ${selectedTire.setPrice}</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, margin: "20px 0" }}>
            {[["In Stock",selectedTire.qty+" available"],selectedTire.tread?["Tread",selectedTire.tread]:["DOT",selectedTire.dot],["Load Index",selectedTire.load],["Speed Rating",selectedTire.speed],["Type",selectedTire.type],["Install Fee","$"+selectedTire.installFee]].map(([k,v]) => <div key={k} style={{ background: COLORS.gray50, borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 11, fontWeight: 600, color: COLORS.gray400 }}>{k}</div><div style={{ fontSize: 14, fontWeight: 700 }}>{v}</div></div>)}
          </div>
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "system-ui, sans-serif", background: "#fff" }}>
      ...
      <div style={{ background: COLORS.orange, padding: "8px 32px", textAlign: "center", fontSize: 14, color: "#fff", fontWeight: 600 }}>🏷️ Free installation on any set of 4 tires — Limited time!</div>
      <div style={{ background: storefront.heroBg, padding: "80px 40px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", maxWidth: 520, margin: "0 auto", background: "rgba(255,255,255,0.1)", borderRadius: 12, padding: 12 }}>
          <input style={{ ...S.input, flex: 1, background: "#fff" }} placeholder="Search by size, brand, or model..." value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <div style={{ padding: "60px 40px", background: COLORS.gray50 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, maxWidth: 900, margin: "0 auto" }}>
          {[["Terrence H.","⭐⭐⭐⭐⭐","Great prices on used tires. In and out in 45 minutes. Will definitely be back!"],["Angela P.","⭐⭐⭐⭐⭐","Reserved online and they had my tires ready when I arrived. Super easy process."],["Devon C.","⭐⭐⭐⭐⭐","Best used tire shop in Greenville. Honest people and fair pricing."]].map(([n,r,t]) => <div key={n} style={S.card}><div style={{ fontWeight: 700, marginBottom: 4 }}>{n}</div><div style={{ marginBottom: 8 }}>{r}</div><div style={{ fontSize: 14, color: COLORS.gray500, lineHeight: 1.6 }}>{t}</div></div>)}
        </div>
      </div>
      ...
      <div style={{ position: "fixed", bottom: 84, right: 24, zIndex: 100 }}>
        {chatOpen && <div style={{ width: 320, background: "#fff", borderRadius: 16, border: "1px solid #E2E8F0", marginBottom: 12, overflow: "hidden" }}>
        ...
        <button onClick={() => setChatOpen(!chatOpen)} style={{ width: 56, height: 56, borderRadius: "50%", background: storefront.primaryColor, border: "none", color: "#fff", fontSize: 24, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>{chatOpen ? "×" : "💬"}</button>
      </div>
  ...
  if (step === 3) return (
    <div style={{ minHeight: "100vh", background: COLORS.navy, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "60px 48px", textAlign: "center", maxWidth: 500 }}>
        ...
        <p style={{ color: COLORS.gray500, marginBottom: 28, lineHeight: 1.7 }}>Your shop account has been created. Your storefront is being set up. You'll receive a confirmation email with next steps.</p>
    ...
    <div style={{ minHeight: "100vh", background: COLORS.navy, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui, sans-serif", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "48px 44px", maxWidth: 520, width: "100%" }}>
        ...
        {step === 1 && <>
          ...
          <div style={{ background: "#F0FDF4", borderRadius: 10, padding: "12px 14px", marginBottom: 20, fontSize: 13, color: "#166534" }}>✓ Invite valid · Plan: Growth Partner · Market: Greenville, SC · Expires May 16, 2026</div>
          <button onClick={() => setStep(2)} style={{ ...S.btn("primary","lg"), width: "100%", justifyContent: "center" }}>Verify & Continue →</button>
        </>}
        {step === 2 && <>
          <div style={{ display: "grid", gap: 12 }}>
            {[["Full Name","Marcus Williams"],["Shop Name","Greenville Tire Pros"],["Password",""],["Confirm Password",""]].map(([l,v]) => <div key={l}><label style={S.label}>{l}</label><input type={l.includes("Password") ? "password" : "text"} style={S.input} defaultValue={v} /></div>)}
          </div>
          <div style={{ background: COLORS.gray50, borderRadius: 10, padding: "12px 14px", marginTop: 16, marginBottom: 20, fontSize: 13, color: COLORS.gray600 }}>Assigned Plan: <strong>Growth Partner — $249/mo</strong><br/>Market: <strong>Greenville, SC</strong></div>
